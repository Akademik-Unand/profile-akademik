...
export function RichTextBlock({ html, bind, box, puck }) {
  const bound = useBoundField(bind);
  return (
    <div className={`px-4 py-6 md:px-6 ${boxToMobileClass(box)}`} style={editorBoxStyle(box, puck)}>
      {bound?.text ? <p className="text-neutral-700">{bound.text}</p> : <HtmlContent html={html} />}
    </div>
  );
}

export function ImageBlock({ image, caption, alt, size, align, bind, box, puck }) {
  const bound = useBoundField(bind);
  ...
  return (
    <figure className={`px-4 py-4 md:px-6 ${width} ${place} ${boxToMobileClass(box)}`} style={editorBoxStyle(box, puck)}>
...
export function GalleryBlock({ title, subtitle, items = [], puck }) {
  const photos = items.filter((item) => item.url || item.image?.url || item.media?.url);
  if (!photos.length && !title) {
    return showEditorChrome(puck) ? <BlockPlaceholder label="Galeri — tambah foto di panel kanan" /> : null;
  }
  return (
    <section className="bg-mist py-16">
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        {title ? <h2 className="font-headline text-3xl text-neutral-900">{title}</h2> : null}
        {subtitle ? <p className="mt-2 max-w-2xl text-sm text-neutral-600">{subtitle}</p> : null}
        <div className="mt-8 grid grid-cols-2 gap-2 lg:grid-cols-4">
          {photos.map((item, index) => (
            <img
              key={item.mediaId || item.url || index}
              src={item.url || item.image?.url || item.media?.url}
              alt={item.caption || ''}
              className="min-h-44 w-full rounded-md object-cover lg:min-h-52"
            />
          ))}
        </div>
      </div>
...
export function ButtonBlock({ label, url, align, variant, bind, box, puck }) {
  const bound = useBoundField(bind);
  ...
  return (
    <div className={`flex w-full px-4 py-3 ${wrap}`}>
      {external ? (
...
export function AccordionBlock({ items = [], puck }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-2 px-4 py-6 md:px-6">
      {rows.map((item, index) => (
...
export function EmbedBlock({ url }) {
...
export function StatsBlock({ items = [], puck }) {
